...
pokemon.post("/", async(req, res, next) => {
    ...
    if(pok_name && pok_height && pok_weight && pok_base_experience){
        ...
        if(rows.affectedRows == 1){
            return res.status(201).json({code: 201 , message: "Pokemos insertado correctamente"});
        }
...
pokemon.put("/:id([0-9]{1,3})", async (re, res, next) =>{
    ...
    if(pok_name && pok_height && pok_weight && pok_base_experience){
        ...
        if(rows.affectedRows == 1){
            return res.status(200).json({code: 200 , message: "Pokemos actualizado correctamente"});
        }
